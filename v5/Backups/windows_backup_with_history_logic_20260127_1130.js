...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function finalizeSuccessUI(liveUrl, slug) {
            ...
            const repoUrl = `https://github.com/mforgedesign/Convite/tree/recuperaçãohoje/convites/${slug}`;
                ...
                publishBtn.addEventListener('click', async () => {
                    ...
                    if (slugExists) {
                        ...
                        confirmed = await showConfirmModal(
                            ...
                            <p class="text-sm text-gray-600">URL: <strong>convite.mforge.com.br/${slug}</strong></p>
                        </div>`,
                            '⚠️ Sobrescrever',
                            'Cancelar'
                        );
                        ...
                        confirmed = await showConfirmModal(
                            'Publicar Convite',
                            `Seu convite será publicado em:<br><strong class="text-brand-600">convite.mforge.com.br/${slug}</strong><br><br>Deseja continuar?`,
                            'Publicar',
                            'Cancelar'
                        );
                    }
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (!templateResp.ok) throw new Error('Template não encontrado');
                            htmlContent = await templateResp.text();
                        } catch (e) {
                            console.warn('[Windows] Fetch failed, trying embedded template...');
                            if (window.EMBEDDED_FINAL_TEMPLATE) {
                                htmlContent = window.EMBEDDED_FINAL_TEMPLATE;
                            } else {
                                throw e;
                            }
                        }
